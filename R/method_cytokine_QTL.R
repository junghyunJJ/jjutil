...
gg_manh<-function(dat, sigth = 5e-08){
  sig <- dat %>% filter(P < sigth) %>% select(SNP) %>% pull
  ...
  ggdat <- dat %>% 
    mutate(bp = unlist(lapply(1:22, function(i){seq(1,raw_ggdat$chr_len[i])}))) %>% 
  ...
  axisdf <- ggdat %>% select(CHR, bpcum) %>% group_by(CHR) %>% summarize(center=( max(bpcum) + min(bpcum) ) / 2 ,.groups = 'drop')
